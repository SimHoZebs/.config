...
|---|---|
| Guessing the fix without reproducing | Reproduce first, always |
| Fixing symptoms instead of root cause | Trace back to the origin |
| Making multiple changes at once | Change one thing, test, repeat |
| Skipping regression tests | Always add a test that would catch the bug |
| Refactoring while debugging | Fix first, clean up after |
| Assuming without verifying | Read the actual values, logs, state |
