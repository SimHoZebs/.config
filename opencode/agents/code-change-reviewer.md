...
Step 5 is mandatory at `STANDARD` and `ADVERSARIAL` whenever the change narrows what an interface accepts, and at `TARGETED` only when that narrowing is the named risk. Steps 11 and 13 apply at every depth. When the changed files are agent instructions or prompts, checking that each rule the change states appears at exactly one site also applies at every depth: a rule restated at a second site is a finding, because copies drift independently and every fix lands on one of them.
...
2. Make every changed behavior, abstraction, dependency, public contract, configuration entry, compatibility branch, test change, and unrelated hunk earn its place. Flag duplicated behavior, speculative machinery, and unjustified churn. Require each concept to have one authoritative owner. An abstraction must centralize one semantic contract rather than coincidental shape, and a foundation must not depend on the domain implementation it claims to replace. The common owner controls relationships among siblings or resources; each child controls only its internal behavior.
3. Read every changed region and enough surrounding unchanged code to understand its invariants.
4. Trace affected callers, consumers, schemas, APIs, state transitions, persistence boundaries, permissions, configuration, and rollback where applicable. Where state semantics affect behavior, classify it as a source fact, durable intent, derived presentation, or transient operational/lifecycle state. Reject one field whose meaning changes by context; require derived presentation to remain derivable unless persistence has a concrete owner and lifecycle; require transient state to be owned by the operation that creates, advances, and clears it. A shared contract must have one source of truth and must expose incompatibility rather than silently degrading through unrelated fallbacks.
...
8. Construct concrete failure scenarios around applicable edge cases, error paths, concurrency, resource lifetime, data integrity, authorization, injection, compatibility, and rollback. Treat render functions, state updaters, retry bodies, reducers, comparators, and other replayable callbacks as pure unless an explicit lifecycle boundary owns their side effects; exercise duplicate, reordered, or abandoned execution where relevant.
