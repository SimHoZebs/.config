...
- Visual primitives and layout ownership
...
## Pass 6: Visual Primitives And Layout Ownership

- Apply the `Visual Primitive And Layout Ownership` catalog checklist. Inspect repeated visual contracts, primitive dependency direction, sibling layout ownership, semantic HTML, and responsive overrides.

## Pass 7: Hygiene Check
...
## Pass 10: Error Handling & Debug Artifacts
...
## Pass 12: Configuration & Build Sprawl
...
- Use only read-only exploration agents for local codebase inspection.
...
- Behavior-preserving ownership or reuse corrections.

**Needs Product Decision**

- Meaning, priority, or interaction choices code inspection cannot settle.

**Do Not Consolidate**

- Similar-looking mechanisms whose semantic or lifecycle differences should remain explicit.
...
## Visual Primitive And Layout Ownership

Smells:

- Repeated badges, stacks, surfaces, controls, links, or filter buttons independently define the same visual contract.
- One-pixel differences in radius, padding, gap, focus rings, or icon alignment arise from separate implementations rather than explicit variants.
- A presentational primitive imports a feature/domain class or depends on a feature stylesheet for its base appearance.
- Children use margins, auto margins, or pseudo-elements to position themselves relative to siblings.
- Parent and child wrappers both impose outer spacing, producing compounded or context-dependent gaps.
- Responsive CSS bypasses the primitive and recreates its layout for one feature.
- A universal component uses many flags to merge elements that only look similar.

Better shapes:

- Give one stable visual contract one owner under a presentational primitive directory such as `components/ui/`.
- Keep domain semantics in thin feature components that compose primitives.
- Use named spacing and control-size variants rather than independent numeric values.
- Let parents own sibling gap, alignment, wrapping, ordering, and separators; let children own internal composition.
- Keep native semantics: links remain links, buttons remain buttons, disclosure summaries remain summaries, even when they share styling.
- Keep intentionally different lifecycle and interaction behavior in separate components over a shared visual foundation.

Verification:

- Search for old classes and duplicated declarations after migration.
- Test semantic elements, accessible names, pressed/disabled/focus states, and variant class composition.
- Check desktop and responsive layouts; typecheck and DOM tests do not prove pixel relationships.
- Run contrast checks when tone or badge primitives change.
